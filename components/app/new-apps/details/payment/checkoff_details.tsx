...
interface CheckOffDetailsProps {
  subsequent?: boolean;
  firstSubsequentDifferent?: boolean;
  onNextAction: () => void;
}

export const CheckOffDetails: FC<CheckOffDetailsProps> = ({
  subsequent = false,
  firstSubsequentDifferent = true,
  onNextAction,
}) => {
  const params = useParams<{ id: string }>();
  const schemaCheckOff = useMemo(
    () => getSchemaCheckOff(subsequent),
    [subsequent]
  );
  const fieldsCheckOff = getFieldsCheckOff(subsequent);
  const labelsCheckOff = getLabelsCheckOff(subsequent);

  type AddInputFormValues = z.infer<typeof schemaCheckOff>;
  const form = useForm<AddInputFormValues>({
    resolver: zodResolver(schemaCheckOff),
    mode: "onChange",
  });

  const [showFields, setShowFields] = useState(fieldsCheckOff);

  const handleRadioChange = (field: string, value: string) => {
    setShowFields((prev) => ({ ...prev, [field]: value }));
  };

  const { selectedProposal, checkOff, documents, loading, submitFeedback } =
    useProposalStore();
  ...
  const onSubmit = async (data: AddInputFormValues) => {
    ...
    for (const field of Object.keys(showFields)) {
      const labelInfo = labelsCheckOff[field as keyof typeof labelsCheckOff];
    ...
    <div className="col-span-3 grid md:grid-cols-2 gap-10">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {Object.values(labelsCheckOff).map((field) => (
            <RadioTextInput
              key={field.name}
              name={field.name}
              label={field.label}
              ...
              description={descriptionCheckOff(
                field.name,
                checkOff,
                selectedProposal
              )}
      ...
      <div className="px-2 py-6">
        {_.isEmpty(documents) && !loading ? (
          ...
          <SubsequentDocuments doctypes={["CHECK_OFF_FORM"]} docUserType="OWNER" />
        ) : (
          <ProposalDocuments doctypes={["CHECK_OFF_FORM", "MANUAL_APPLICATION_FORM"]} docUserType="OWNER" />
        )}
      </div>
    </div>
  );
