...
import { z } from "zod";
import {
  descriptionDirectDebit,
  getFieldsDirectDebit,
  getLabelsDirectDebit,
  getSchemaDirectDebit,
} from ".";
import {
  ProposalDocuments,
  RadioTextInput,
...
export const DirectDebitDetails: FC<DirectDebitDetailsProps> = ({
  subsequent = false,
  firstSubsequentDifferent = true,
  onNextAction,
}) => {
  const params = useParams<{ id: string }>();
  const schemaDirectDebit = useMemo(
    () => getSchemaDirectDebit(subsequent),
    [subsequent]
  );
  const fieldsDirectDebit = getFieldsDirectDebit(subsequent);
  const labelsDirectDebit = getLabelsDirectDebit(subsequent);

  type AddInputFormValues = z.infer<typeof schemaDirectDebit>;
  const form = useForm<AddInputFormValues>({
    resolver: zodResolver(schemaDirectDebit),
    mode: "onChange",
  });

  const [showFields, setShowFields] = useState(fieldsDirectDebit);
  ...
  const onSubmit = async (data: AddInputFormValues) => {
    ...
    for (const field of Object.keys(showFields)) {
      const labelInfo =
        labelsDirectDebit[field as keyof typeof labelsDirectDebit];
    ...
    <div className="col-span-3 grid md:grid-cols-2 gap-10">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {Object.values(labelsDirectDebit).map((field) => (
            <RadioTextInput
              key={field.name}
              name={field.name}
              label={field.label}
              ...
              description={descriptionDirectDebit(
                field.name,
                directDebit,
                personalDetail,
                selectedProposal
      ...
      <div className="px-2 py-6">
        {_.isEmpty(documents) && !loading ? (
          ...
          <SubsequentDocuments doctypes={["DIRECT_DEBIT_FORM"]} docUserType="OWNER" />
        ) : (
          <ProposalDocuments doctypes={["DIRECT_DEBIT_FORM", "MANUAL_APPLICATION_FORM"]} docUserType="OWNER" />
        )}
      </div>
    </div>
  );
