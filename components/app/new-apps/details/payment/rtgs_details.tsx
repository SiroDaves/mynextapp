...
interface RtgsDetailsProps {
  subsequent?: boolean;
  firstSubsequentDifferent?: boolean;
  onNextAction: () => void;
}

export const RtgsDetails: FC<RtgsDetailsProps> = ({
  subsequent = false,
  firstSubsequentDifferent = true,
  onNextAction,
}) => {
  const params = useParams<{ id: string }>();
  const schemaRtgs = useMemo(() => getSchemaRtgs(subsequent), [subsequent]);
  const fieldsRtgs = getFieldsRtgs(subsequent);
  const labelsRtgs = getLabelsRtgs(subsequent);

  type AddInputFormValues = z.infer<typeof schemaRtgs>;
  const form = useForm<AddInputFormValues>({
    resolver: zodResolver(schemaRtgs),
    mode: "onChange",
  });

  const [showFields, setShowFields] = useState(fieldsRtgs);
  ...
  const onSubmit = async (data: AddInputFormValues) => {
    ...
    for (const field of Object.keys(showFields)) {
      const labelInfo = labelsRtgs[field as keyof typeof labelsRtgs];
    ...
    <div className="col-span-3 grid md:grid-cols-2 gap-10">
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          ...
          {Object.values(labelsRtgs).map((field) => (
            <RadioTextInput
              key={field.name}
              name={field.name}
              label={field.label}
      ...
      <div className="px-2 py-6">
        {_.isEmpty(documents) && !loading ? (
          ...
          <SubsequentDocuments doctypes={["RTGS"]} docUserType="OWNER" />
        ) : (
          <ProposalDocuments doctypes={["RTGS", "MANUAL_APPLICATION_FORM"]} docUserType="OWNER" />
        )}
      </div>
    </div>
  );
